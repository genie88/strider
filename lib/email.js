...
exports.notify_email_change = function (user, old_email) {
  var current_time = new Date();
  var subject = "[Raptor] - 邮箱地址变更提醒 - " + current_time.getHours() + ":" + current_time.getMinutes() ;
...
exports.send_invite_collaborator_new_user = function(inviter, email, code, url) {
  ...
  var subject = "[Raptor] 邀请 " + display_name + '加入Raptor';
  var to = email;
...
exports.notify_new_admin = function (user) {
  var current_time = new Date();
  var subject = "[Raptor] - 新增管理员  - " + user + " - " + current_time.getHours() + ":" + current_time.getMinutes() ;
  var body_template = "超级管理员，你好！\n\n" +
    "Raptor 新增管理员: " + user + "\n\n" +
    " - ci.mgtv.com\n" +
    "Raptor 芒果TV前端CI&CD系统";

    var body = body_template;

    var to = "chenliang2@mgtv.com";
    mailer.send(to, subject, body, body);
    console.log("Sending admin notification for new admin " + user);
}

exports.notify_job_result = function (collaborators, job) {
  var current_time = new Date();
  var result = job.code == 0 ? '成功': '失败';
  var subject = "[Raptor] - 项目 - " + job.project + '构建' + result + '！ 更多信息请查看邮件' ;
  var body_template = "你好！\n\n" +
    "[Raptor] - 项目 - " + job.project + '构建' + result + "\n\n" +
    "构建编译信息: " + job.stdout + "\n\n" +
    "在网页打开编译信息: " + job.jobUrl + "\n\n" +
    " - ci.mgtv.com\n" +
    "Raptor 芒果TV前端CI&CD系统";

  var body = body_template;
  mailer.send(collaborators, subject, body, body);
  console.log("Sending job result to  collaborators: " + collaborators);
}
