  ...
  jobDone: function (emitter, data) {
    ...
    Job.findById(data.id, function (err, job) {
      ...
      Project.findOne({name: job.project}).lean().exec(function (err, project) {
        ...
        //发送构建结果邮件给相关方
        // console.log(job);
        // job.project.name : chenliang/ci-test
        // job._id: 57569665ef30c2a61999637f
        // 构建结果查看页地址 ：http://ci.mgtv.com/chenliang/ci-test/job/57569665ef30c2a61999637f
        // 错误信息 job.std.merged
        ...
        if (env === 'production') {
          ...
          var info = {
            ...
            jobUrl: config.server_name + '/' + job.project.name + '/job/' + job._id,
            stdout: job.std.merged
          }
          // User.findById(project.creator, function(err, creator){
          //   if(err || !creator) return;
          //   mailer.notify_job_result(creator.email, info);
          // })
          ...
          User.collaborators(job.project.name, 0, function (err, users) {
            ...
            for (var i = 0; i < users.length; i++) {
              collaborators.push(users[i].email);
            }

            mailer.notify_job_result(collaborators, info);
          });
        }
